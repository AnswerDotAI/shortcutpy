...
Pathish = str | Path
DictItem = tuple[str, "Expr"]
Block = list["Stmt"]
BlockResult = tuple[Block, set[str]]
...
def compile_file(src: Pathish, output: Pathish | None = None, *, sign: bool = True, mode: str = "people-who-know-me", keep_unsigned: bool = False) -> ShortcutArtifact:
    ...
    output = Path(output) if output else src.with_suffix(".shortcut")
    if sign:
...
class Lowerer:
    ...
    def lower(self, tree: ast.Module) -> ShortcutProgram:
        ...
        body,_ = self.lower_block(fn.body, set())
    ...
    def lower_block(self, stmts: list[ast.stmt], assigned: set[str]) -> BlockResult:
        body,current = [],set(assigned)
        for stmt in stmts:
            if isinstance(stmt, ast.Assign):
                body.append(self.lower_assign(stmt, current))
                current.add(stmt.targets[0].id)
                continue
            if isinstance(stmt, ast.Expr):
                if not isinstance(stmt.value, ast.Call): raise self.error("Only action calls may be used as bare expressions", stmt)
                body.append(ExprStmt(self.lower_call(stmt.value, current)))
                continue
            if isinstance(stmt, ast.If):
                pred = self.lower_predicate(stmt.test, current)
                then_body,then_assigned = self.lower_block(stmt.body, current)
                else_body,else_assigned = self.lower_block(stmt.orelse, current) if stmt.orelse else ([], set(current))
            ...
            if isinstance(stmt, ast.For):
                loop = self.lower_for(stmt, current)
                body.append(loop)
                continue
            if isinstance(stmt, ast.Return):
                body.append(Return(self.lower_expr(stmt.value, current) if stmt.value else None))
                continue
            raise self.error(f"Unsupported statement: {stmt.__class__.__name__}", stmt)
        return body,current

    def lower_assign(self, stmt: ast.Assign, assigned: set[str]) -> Assign:
        if len(stmt.targets) != 1 or not isinstance(stmt.targets[0], ast.Name):
            raise self.error("Only simple name assignment is supported", stmt)
        return Assign(stmt.targets[0].id, self.lower_expr(stmt.value, assigned))

    def lower_for(self, stmt: ast.For, assigned: set[str]) -> RepeatEach | RepeatCount:
        ...
        if isinstance(stmt.iter, ast.Call) and isinstance(stmt.iter.func, ast.Name) and stmt.iter.func.id == "range":
            ...
            body,_ = self.lower_block(stmt.body, inner_assigned)
            return RepeatCount(target, count, body)
        source = self.lower_expr(stmt.iter, assigned)
        body,_ = self.lower_block(stmt.body, inner_assigned)
        return RepeatEach(target, source, body)

    def lower_predicate(self, node: ast.expr, assigned: set[str]) -> Predicate:
        if isinstance(node, ast.Compare):
            ...
            return Predicate(self.lower_expr(node.left, assigned), op_str, self.lower_expr(node.comparators[0], assigned))
    ...
    def lower_expr(self, node: ast.expr, assigned: set[str]) -> Expr:
        if isinstance(node, ast.Name):
            if node.id not in assigned: raise self.error(f"Unknown name: {node.id}", node)
        ...
        if isinstance(node, ast.List): return ListExpr([self.lower_expr(o, assigned) for o in node.elts])
        if isinstance(node, ast.Dict):
            items = []
            for key,value in zip(node.keys, node.values):
                if key is None: raise self.error("Dictionary unpacking is not supported", node)
                items.append((self.literal_text_key(key), self.lower_expr(value, assigned)))
            return DictExpr(items)
        if isinstance(node, ast.Call): return self.lower_call(node, assigned)
        raise self.error(f"Unsupported expression: {node.__class__.__name__}", node)

    def lower_call(self, node: ast.Call, assigned: set[str]) -> CallExpr:
        ...
        for kw in node.keywords:
            ...
            kwargs[kw.arg] = self.lower_expr(kw.value, assigned)
        call = CallExpr(node.func.id, [self.lower_expr(arg, assigned) for arg in node.args], kwargs)
    ...
    def literal_text_key(self, node: ast.expr) -> str:
        value = self.literal_value(node, "dictionary key")
        if not isinstance(value, str): raise self.error("Dictionary keys must be string literals", node)
        return value
...
class Emitter:
    ...
    def emit(self, program: ShortcutProgram) -> dict[str, Any]:
        ...
        return PAYLOAD_TEMPLATE | dict(WFWorkflowActions=self.actions, WFWorkflowIcon=icon, WFWorkflowInputContentItemClasses=DEFAULT_INPUT_CLASSES.copy())
    ...
    def dictionary_item(self, key: str, value: Expr) -> dict[str, Any]:
        item = self.list_item(value)
        return dict(WFItemType=item["WFItemType"], WFKey=self.text_token(key), WFValue=item["WFValue"])
    ...
    def raw_value(self, expr: Expr) -> Any:
        match expr:
            ...
            case DictExpr(items=items):
                return {k: self.raw_value(v) for k,v in items}
